...
use App\Traits\PatientAttendantAddressProofValidation;

class PatientAttendantAddressProofService implements CRUDContract, FilterContract
{

    use PatientAttendantAddressProofValidation;
     ...
     * @param string $id
     * @return PatientAttendantAddressProof|null
     */
    public function getDataByDynamicColumnsName(string $columnsName = 'patient_id', string $id): mixed
    {
        return PatientAttendantAddressProof::where($columnsName, $id)->first();
    }
     ...
     * @return PatientAttendantAddressProof
     */
    private function createRecordOrUpdate(Request $request, string|null $id): PatientAttendantAddressProof
    {
        $proof = PatientAttendantAddressProof::where('patient_id', $request->patient_id)->first();
        if ($proof) {
            ...
            $proof = PatientAttendantAddressProof::create(
                [
                    'consent_given_at' => now(),
                    'id_type' => $request->id_type,
                    'consent' => $request->consent,
     ...
     * @return PatientAttendantAddressProof
     */
    private function recordCreate(Request $request): PatientAttendantAddressProof
    {
        $this->checkValidationService->checkValidation($this->validate($request));
    ...
    {
        return PatientAttendantAddressProof::where('patient_id', $id)->first();
    }
